"""Independent staged CUDA-CP ranking and fixed-K LP audit.
...
def main():
    ...
    for mapping in (manifest["inputs_sha256"], summary["outputs_sha256"]):
        for path, expected in mapping.items():
            bind(path, expected)
    for path in (args.native, args.family_audit, args.initial, args.initial_phase1):
    ...
    for path, expected in family["inputs_sha256"].items():
        bind(path, expected)
    require(family_bindings.get(args.native.resolve()) == bind(args.native) and
            family_bindings.get(args.initial.resolve()) == bind(args.initial), "Family proof base/payload mismatch")
    gpu_audit_path = RESULTS / "20260916_cp_gpu_controls/audit.json"
    ...
    for directory in ("20260916_whole_matching_pilot", "20260916_matching_hint_shortlist"):
        for path in sorted((RESULTS / directory / "probes").glob("*_candidate.json")):
            ...
            excluded.add(sig)
            previous.append(dict(candidate_path=path.relative_to(ROOT).as_posix(), candidate_sha256=bind(path), overlap_edges_sha256=edge_hash(sig)))
    require(len(previous) == len(excluded) == 193 and previous == manifest["previous_candidates"], "Previous candidate exclusions differ")
    native = load(args.native)
    options = native.pop("overlap_candidates")
    moves, keys = native["moves"], [signature(k) for k in options]
    require(len(keys) == len(moves) == family["legal_count"] == manifest["coarse_candidates"] == 73239 and
            len(set(keys)) == len(keys), "Wrong audited family size")
    del options
    stage_reports = []

    def stage(stem, indices, steps, baseline):
        record, payload = load(args.run / (stem + "_stage.json")), load(args.run / (stem + "_gpu.json"))
    ...
    require((manifest["coarse_steps"], manifest["refine_steps"], manifest["refine_count"], manifest["lp_count"]) == (500, 2000, 2048, 64), "Unexpected finite study size")
    coarse, coarse_rows = stage("coarse", range(len(keys)), 500, True)
    del coarse_rows
    refine_expected = selection(list(coarse), coarse, moves, 2048, 140)
    require(load(args.run / "refine_selection.json") == refine_expected, "Coarse-to-refine selection/roles differs")
    refined, refined_rows = stage("refined", [r["proposal_index"] for r in refine_expected], 2000, True)
    eligible = [i for i in refined if keys[i] not in excluded]
    expected = selection(eligible, refined, moves, 64, 32)
    ...
    require(load(args.run / "lp_selection.json") == expected, "Refine-to-LP selection/roles differs")
    selected = [r["proposal_index"] for r in expected]
    replay_scores, gpu_vectors = stage("selected_vectors", selected, 2000, False)
    require(all(abs(replay_scores[i] - refined[i]) <= 1e-9 for i in selected), "Selected rerun ranking differs")
    require(len(summary["records"]) == summary["probes"] == 64 and len(list((args.run / "probes").glob("*_candidate.json"))) ==
            len(list((args.run / "probes").glob("*_phase1.json"))) == 64, "Wrong LP inventory")
    replay_indices = []
    ...
    for chosen, record in zip(expected, summary["records"]):
        ...
        if i in replay_indices:
            cpu = cpu_replay(rows, x0, y0, 2000)
            errors = {field: max(abs(a - b) for a, b in zip(values, point[field])) for field, values in cpu.items()}
            require(max(errors.values()) <= 1e-8, "Independent2000-step CPU/GPU vectors differ")
            max_vector_error = max(max_vector_error, *errors.values())
            cpu_reports.append(dict(proposal_index=i, cycle_partition=list(partition(moves[i])), steps=2000,
    ...
    require(summary["best"] == numeric_order[0] and summary["numerically_improving_candidates"] == improving and
            summary["numerical_improvement_count"] == len(improving) and summary["numerically_optimal_count"] == 64,
    ...
    for path in (Path(__file__), ROOT / "acceleration/audit_certificate.py", ROOT / "acceleration/audit_phase1.py", ROOT / "acceleration/audit_phase1_kkt.py"):
        bind(path)
    report = dict(status="INDEPENDENT_CP_MATCHING_SEARCH_AUDIT_PASS", inputs_sha256=hashes,
                  ...
                  eligible_refined_candidates=len(eligible), selected_distinct_new_LP_candidates=64,
                  both_selection_stages_indices_roles_and_upper_only_ties_replayed=True, stages=stage_reports,
                  selected_saved_vectors_finite_and_exact_box_feasible=True, selected_full99_primal_dual_checkpoint_evaluations=128,
                  max_selected_cp_scalar_absolute_error=max_scalar_error, cpu_replays=cpu_reports,
                  max_2000_step_cpu_gpu_vector_absolute_error=max_vector_error,
                  actual_LP_exact_intervals_checked=64, actual_LP_exact_positive_lower_bounds=sum(r["phase1_audit"]["exact_positive_dual_bound"] for r in reports),
                  ...
                  scope="Coarse/refined rankings replay saved numerical upper values only; prior193 candidates are excluded at the LP shortlist stage, not at coarse/refinement. Exact scalar/input associations checked for all stages; independent full99 CP evaluation only for64 selected vectors and2000-step CPU replay for3 selected shapes. Exact positive LP lower bounds exclude only their own K. No pair/domain pass, exhaustive LP coverage, exact CP proof, graph construction or general nonexistence claim.")
