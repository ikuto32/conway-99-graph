...
def main():
    ...
    modelmeta=read(MODEL/'model.json');primary=read(TABLES/'manifest.json');read(TABLES/'summary.json');labels=[(2*a+s,2*b+t)for a,b in combinations(range(7),2)for s in range(2)for t in range(2)]
    ...
    for u in range(84):
        original=read(TABLES/f'domain_{u:02d}.json')['domain_masks_hex'];ids=modelmeta['retained_original_domain_ids'][u];h.require(len(ids)==len(set(ids))and ids==sorted(ids)and all(0<=i<len(original)for i in ids),'retained IDs');table=[original[i]for i in ids];known={v-15 for v in B[u+15]if v>=15};values=[]
        for text in table:
