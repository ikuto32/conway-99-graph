"""Version2 CP search: explicit family identity, raw-dual guards, prior-summary exclusions.
...
def main():
    ...
    require(data['status'] == 'COMPLETE_WHOLE_SAME_SIGN_MATCHING_SUBFAMILY_ENUMERATION' and
            len(candidates) == len(moves) == family['legal_count'] and data['selector'] == family['selector'] == 'all', 'Family size/scope mismatch')
    ...
    if args.validate_only:
        print(json.dumps(dict(status='CP_MATCHING_V2_INPUT_VALIDATION_PASS', producer_version=2, paths=paths,
    ...
    output(args.out/'manifest.json', dict(status='CP_MATCHING_SEARCH_MANIFEST', producer_version=2, paths=paths,
           family_association=family_association, previous_summary_reports=previous_summary_reports,
    ...
    summary = dict(status='BOUNDED_CP_MATCHING_SEARCH_FINISHED', producer_version=2, paths=paths,
                   family_association=family_association, previous_summary_reports=previous_summary_reports,
