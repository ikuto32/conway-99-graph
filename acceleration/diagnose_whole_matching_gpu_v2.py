...
def main():
    ...
    geometry_best = {group["best_proposal_index"] for grouping in ("by_move_category", "by_changed_edge_count", "by_cycle_sizes") for group in statistics[grouping].values()}
    checked = sorted(set(ordered[:20])|set(coordinate_best.values())|set(random_sample)|geometry_best)
    ...
    for subset_index, proposal_index in enumerate([None]+checked):
        ...
        expected = evaluate({"overlap_edges_outer_zero_based":list(map(list,edges))},x)
    ...
    report = dict(status="WHOLE_MATCHING_FIXED_X_GPU_DIAGNOSTIC_AND_INDEPENDENT_SAMPLE_AUDIT_PASS",
                  ...
                  selection_rule="Unchanged baseline + top20 + each of14coordinate best + each move-category/changed-edge-count/cycle-partition best + fixed-seed uniform sample, deduplicated",
                  top20_indices=ordered[:20],coordinate_best_indices=coordinate_best,random_sample_indices=random_sample,geometry_best_indices=sorted(geometry_best),
