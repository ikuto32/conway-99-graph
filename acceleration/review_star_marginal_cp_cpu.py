...
from star_marginal_cp_cpu import (build_model, diagonal_steps, factored_forward,
    factored_transpose, numeric_bounds, product_simplex_projection, simplex_projection, step)
...
def main():
    ...
    def bind(path, expected=None):
        ...
        bindings[name] = value
        return value
    for name in (__file__, 'acceleration/star_marginal_cp_cpu.py', 'acceleration/audit_certificate.py'):
        bind(name)
    ...
    for index in (25496, 26025):
        ...
        y0 = np.zeros(5166)
        runs = []
        for start_name, initial_p, initial_y, checkpoints in (
        ...
        cases.append(dict(index=index, candidate_path=key(candidate_path), phase_path=key(phase_path),
            ...
            model_build_seconds=build_seconds, per_operation_cpu_timings=timings, runs=runs))
    require(all(digest(name) == expected for name, expected in bindings.items()), 'Input/source changed during study')
    report = dict(status='STAR_SIMPLEX_PDHG_CPU_REFERENCE_AND_BOUNDED_QUALITY_CONTROLS_PASS',
        ...
        scope='Two saved fixed-K controls only. Existing independently exact intervals are hash-bound references; CP floating values are numerical diagnostics. No CUDA implementation, batch research search, graph construction, or new exclusion claim.',
