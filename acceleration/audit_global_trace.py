"""Independent global-defect search replay, proposal and numerical merit audit.
...
def main():
    ...
    require(summary['status'] == 'BOUNDED_GLOBAL_DEFECT_SEARCH_FINISHED', 'Search is not finished')
    ...
    for number, record in enumerate(summary['records']):
        ...
        options, trades = proposal['overlap_candidates'], proposal['trades']
        require(len(options) == len(trades) == min(proposal['legal_trades'], manifest['neighbors']), 'Proposal counts differ')
        require(len({signature(edges) for edges in options}) == len(options), 'Duplicate proposed candidates')
        for edges, move in zip(options, trades):
            require(signature(trade(current, move['removed'], move['added'])) == signature(edges), 'Proposed graph differs from trade')
        ...
        require(chosen['usable'] and chosen['objective'] == min(info['objective'] for info in trial_infos if info['usable']),
                'Chosen merit is not the best usable actual LP merit')
        require(record['removed'] == trades[record['chosen_index']]['removed']
                and record['added'] == trades[record['chosen_index']]['added'], 'Chosen trade differs')
        delta = chosen['objective']-current_objective
        require(record['accepted'] or delta > 1e-7, 'Improving/equal trial was unexpectedly rejected')
        require(not record['accepted'] or delta <= 0.5, 'Accepted increase exceeds escape cap')
        if record['accepted']:
            current = trade(current, record['removed'], record['added'])
    ...
    for path in sorted((args.run/'proposals').glob('iteration_*.json')):
        ...
        require(len(proposal['overlap_candidates']) == len(proposal['trades']), 'Terminal proposal count mismatch')
        for edges, move in zip(proposal['overlap_candidates'], proposal['trades']):
            require(signature(trade(current, move['removed'], move['added'])) == signature(edges), 'Terminal proposed graph/trade mismatch')
            full_graph({'overlap_edges_outer_zero_based': edges})
            all_proposal_graphs += 1
    ...
    require(summary['stop_reason'] in ('ITERATION_LIMIT', 'NUMERICAL_ZERO_DEFECT_REQUIRES_EXACT_VALIDATION',
            'NO_LEGAL_TWO_EDGE_MOVE', 'SAMPLED_NEIGHBORS_ALREADY_VISITED', 'ALL_SELECTED_PHASE1_PROBES_INCOMPLETE'), 'Unknown stop reason')
    ...
    for source in (Path(__file__), ROOT/'acceleration/audit_phase1.py', ROOT/'acceleration/audit_certificate.py',
        ...
        digest(source)
    result = {'status': 'INDEPENDENT_GLOBAL_TRACE_PROPOSALS_AND_NUMERIC_OBJECTIVES_AUDIT_PASS',
              ...
              'scope': 'Every saved proposal graph/trade and actual LP candidate/model/numerical merit is checked. Every usable LP gets exact rational primal/dual bounds; a strictly positive lower bound excludes only its fixed candidate. Accepted-state/cache/best provenance is replayed. This is not proof of exact optimum, zero-defect exact feasibility, a completed graph, all sampled-neighbor exclusion, or global coverage.'}
