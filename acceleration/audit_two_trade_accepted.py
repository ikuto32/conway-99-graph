"""Independent two-trade coupled-search intervals, allowing recorded uphill moves.
...
def main():
    ...
    require(summary["status"] == "BOUNDED_TWO_TRADE_COUPLED_SEARCH_FINISHED", "Search not finalized")
    require(summary["manifest"]["trades_per_move"] == 2, "Wrong atomic trade count")
    ...
    for record in records:
        ...
        require(type(record["trade_path"]) is list and len(record["trade_path"]) == 2,
                "Accepted move does not contain two atomic trades")
        intermediate = set(current_edges)
        for trade in record["trade_path"]:
            removed, added = set(map(tuple, trade["removed"])), set(map(tuple, trade["added"]))
            require(len(removed) == len(added) == 2 and removed <= intermediate and not added & intermediate,
                    "Invalid accepted atomic trade")
            intermediate = intermediate - removed | added
            # The intermediate need not pass pair AC, but it must be a legal partial graph.
            full_graph({"overlap_edges_outer_zero_based": [list(edge) for edge in sorted(intermediate)]})
        require(intermediate == next_edges, "Two trades do not produce chosen graph")
    ...
    require(summary["accepted_atomic_trades"] == 2*len(accepted), "Atomic-trade summary mismatch")
    ...
    report = dict(status="INDEPENDENT_TWO_TRADE_ACCEPTED_PHASE1_INTERVAL_AUDIT_PASS",
                  accepted_moves_checked=len(accepted), distinct_phase1_audits=len(audits),
                  accepted_atomic_trades_checked=2*len(accepted),
                  intermediate_states_independently_pass_full99_partial_caps=True,
