"""Independent balanced-coordinate whole-family128 mapping check; no producer imports.

This checks the separately preregistered replacement for the superseded first
lexicographic-stratum request; that old request is never approved here.
"""
import argparse
...
def choose_independently(scores, strata, excluded):
    ...
    coordinates = sorted({s[:2] for s in buckets})
    require(coordinates == [(g, 'same_'+str(sign)) for g in range(7) for sign in range(2)], 'all14 diversity coordinates')
    # Precompute horizontal rounds through each coordinate's fixed sorted
    # shape buckets. This uses no mutable cursor or deque from the producer.
    streams = {}
    for coordinate in coordinates:
        groups = [buckets[s] for s in sorted(buckets) if s[:2] == coordinate]
        streams[coordinate] = [group[depth] for depth in range(max(map(len, groups)))
                               for group in groups if depth < len(group)]
    extra = [streams[c][depth] for depth in range(max(map(len, streams.values())))
             for c in coordinates if depth < len(streams[c])][:64]
    require(len(extra) == 64 and {strata[i][:2] for i in extra} == set(coordinates), 'balanced diversity coverage')
...
def validate_request(request, selected, scores, positions, strata, family):
    ...
    require(request['selection_policy']['name'] == '64_BEST_REFINED_UPPER_THEN64_BALANCED_COORDINATES_ROTATING_CYCLE_SHAPES_V2B' and
            request['selection_policy']['global_count'] == request['selection_policy']['stratified_count'] == 64 and
            request['selection_policy']['sampling_is_not_exhaustive'] is True, 'balanced policy identity')
    ...
    for position, (i, row) in enumerate(zip(selected, request['records'])):
        ...
        expected_role = 'global_refined_upper64' if position < 64 else 'balanced_coordinate_then_cycle_shape64'
...
def main():
    ...
    def read(p):
        ...
        return d
    for f in (__file__, 'acceleration/rank_fresh_whole_v2_balanced.py', 'acceleration/rank_fresh_star_pdhg.py', args.preregistration, 'uv.lock'): bind(f)
    request = read(args.request); preflight = read(args.preflight); producer_controls = read(args.producer_controls)
    deviation_path = path(args.request).parent/'protocol_deviation.json'
    deviation = read(deviation_path)
    old_request = read(deviation['old_request_path'])
    bind(deviation['old_request_path'], deviation['old_request_sha256'])
    bind(args.request, deviation['new_request_sha256'])
    for f, h in deviation['source_sha256'].items(): bind(f, h)
    require(deviation['status'] == 'PRE_GPU_SELECTION_PROTOCOL_REVISION' and
            deviation['native_processes'] == deviation['GPU_processes'] == deviation['LP_runs'] == 0 and
            deviation['thresholds_changed'] is False and deviation['iterations_unchanged'] == 500 and
            old_request['indices'][:64] == request['indices'][:64] and
            len(set(old_request['indices']) & set(request['indices'])) == deviation['request_overlap_candidate_count'] == 99,
            'preserved pre-result protocol revision')
    expected_control_names = ['ACTUAL128_REQUEST','DUPLICATE_SELECTION_INDEX','ALREADY_LP_EVALUATED_SELECTION',
        'FAKE_CROSS_FAMILY_STATUS','CHANGED_ORIGINAL_NATIVE_ROW_ID','CHANGED_CYCLE_SHAPE','CHANGED_MATCHING_CLASS',
        'CHANGED_RAW_GRAPH_SHA','CHANGED_RAW_CANDIDATE_FILE_SHA','CHANGED_REFINED_SCORE','FILTERED_OBJECTIVE_SUBSTITUTION',
        'CHANGED_ITERATION_CAP','CHANGED_SOURCE_BINDING','CORRUPTED_ALTERNATING_CYCLE','UNREVIEWED_EXECUTION_GATE']
    require(producer_controls['status'] == 'WHOLE_FRESH_V2_PRODUCER_MAPPING_CONTROLS_PASS' and
            producer_controls['control_count'] == 15 and
            [r['name'] for r in producer_controls['controls']] == expected_control_names and
            [r['outcome'] for r in producer_controls['controls']] == ['ACCEPT']+['REJECT']*14 and
            producer_controls['native_processes'] == producer_controls['GPU_processes'] == producer_controls['LP_runs'] == 0 and
            producer_controls['independent_verification'] is False, 'producer control record integrity')
    require(producer_controls['inputs_sha256'][key(args.request)] == bind(args.request) and
            producer_controls['inputs_sha256']['acceleration/qa_fresh_whole_v2_balanced.py'] == bind('acceleration/qa_fresh_whole_v2_balanced.py'),
            'producer controls exact request/source binding')
    ...
    for name in ('off_by_one_native_index', 'wrong_cycle_shape', 'wrong_refined_row', 'duplicate_selected_index', 'wrong_selection_role', 'swapped_ranked_diverse_order'):
        ...
        if name == 'wrong_cycle_shape': bad['records'][0]['alternating_cycle_sizes'] = [99]
        ...
        if name == 'wrong_selection_role': bad['records'][0]['roles'] = ['balanced_coordinate_then_cycle_shape64']
    ...
    report = dict(status='INDEPENDENT_WHOLE_FRESH_V2_MAPPING_PASS', timestamp=datetime.now(timezone.utc).isoformat(),
        ...
        producer_control_records_checked=15, protocol_deviation_path=key(deviation_path),
        preregistration_review='Exact balanced selection, exclusions, original-domain objective,500iterations and unavailable-case treatment agree with reviewed source; ranking remains a separate audit obligation',
        preregistration_path=key(args.preregistration), selection='64 smallest refined upper then64 round-robin all14 coordinates, with each coordinate cycling its sorted component-size buckets; exact graph duplicate exclusions',
        diversity_coordinate_counts=[dict(root_group=g, matching_class=kind,
            count=sum(strata[i][:2] == (g, kind) for i in selected[64:])) for g in range(7) for kind in ('same_0', 'same_1')],
        policy_version='BALANCED_COORDINATES_V2B', superseded_first_policy_approved=False,
