"""Independent complete enumeration for four sign coordinates at root groups zero and one."""
...
ROOT=h.ROOT;D=ROOT/'acceleration/results/20260917_partial_four_matchings';O=ROOT/'acceleration/results/20260917_independent_review/four_matchings';OLD=ROOT/'acceleration/results/20260917_partial_two_matchings'
def main():
    ...
    oldproofpath=ROOT/'acceleration/results/20260917_independent_review/two_matchings/summary.json';h.require(h.digest(oldproofpath)=='f3d2ba90be5e8c27bc64c2e4941174d36fc3ddfcbc29ce25da872df652383acb','old audit');read(oldproofpath)
    ...
    h.require(len(Y)==240 and len(removed)==24 and len(fixed)==144 and len(unknown)==1920 and len(affected)==44,'scope dimensions')
    ...
    for u in tqdm(range(84),desc='Independent four-coordinate domains',unit='center'):
    ...
    count=sum(map(len,tables));embedded=sum(r['old_embedded_choices']for r in reports);h.require(count==290460==summary['complete_domain_choices']and embedded==89308==summary['old_embedded_choices'],'population totals')
    result=dict(status='INDEPENDENT_FOUR_COORDINATE_DOMAINS_PASS',claim_id='C-PARTIAL-K-FOUR-COORDINATE-DOMAINS',claim_revision=1,recommendation='VERIFIED',statement='The frozen tables contain exactly all locally admissible degree14 stars satisfying full99 upper common-neighbor caps and exact14 root quotas in the declared four-coordinate scope; every prior two-coordinate star embeds with identical full outer neighborhood.',started_at=started,completed_at=datetime.now(timezone.utc).isoformat(),source_commit=head,command=[sys.executable,*sys.argv],cwd=str(ROOT),python=platform.python_version(),inputs_sha256=bindings,records=reports,domain_choices=count,old_embedded_choices=embedded,centers=84,fixed_K_edges=144,freed_matching_edges=240,unknown_edges=1920,affected_centers=44,missing_neighbor_histogram={k:sum(14-B[u+15].bit_count()==k for u in range(84))for k in (8,9,10)},controls=controls,producer_imported=False,shared_components=['Python standard library','tqdm','prior independent multiway demand-subset enumerator and full99 checker'],scope='Both same-sign coordinates at rootgroups0and1 freed; exactly144 baseline K edges and all other prescribed absences retained. No nontrivial automorphism assumption.',limits=dict(seconds=600,nodes_per_center=5000000),limitations=['Local domains alone do not establish global feasibility or exclusion.','No broader K-family coverage or count asserted.'])
