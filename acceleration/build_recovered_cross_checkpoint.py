"""Index an explicitly repaired cross3/4 CP LP view without rewriting a stopped round.
...
def check_recovery_semantics(original, view, audit, repairs):
    ...
    require(original['status'] == 'BOUNDED_CP_MATCHING_SEARCH_FINISHED' and
            ...
            audit['status'] == 'INDEPENDENT_CP_CROSS_REPAIRED_LP_AUDIT_PASS' and
...
def inspect_recovery(book, directory, recovery):
    ...
    for p in (view_path,original_path,view['repair_manifest_path'],ROOT/'acceleration/audit_cp_cross_recovery.py'):
        book.assert_bound(audit,p)
    for p in (directory/'failure.json',directory/'logs/05_search_audit.log',original_path):
        book.assert_bound(repairs,p)
    require('Phase-I primal-dual gap too large' in (directory/'logs/05_search_audit.log').read_text(encoding='utf-8'),
...
def build(args):
    ...
    book.assert_bound(manifest,ROOT/'acceleration/evaluate_recovered_cross_star.py')
