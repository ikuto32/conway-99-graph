...
def inspect_artifact(candidate_path, result_path, tolerance=1e-7):
    ...
    # Uncrossed IPM points can lie close to a nonsmooth kink. A thresholded
    # derivative diagnostic need not be small even when the exact gap is tiny.
    # The correct continuous complementary slacks remain exact and nonnegative.
    exact_x = list(map(Fraction, clipped_x))
    exact_residuals = [sum((exact_x[e] for e in row["terms"]), Fraction(0)) - row["target"]
                       for row in groups]
    row_complementarity = [(abs(r) if row["equality"] else max(0, r)) - weight*r
                           for row, r, weight in zip(groups, exact_residuals, clipped_y)]
    box_complementarity = [c*value-min(0, c) for value, c in zip(exact_x, combined)]
    require(all(value >= 0 for value in row_complementarity + box_complementarity),
            "Negative exact complementarity slack")
    require(sum(row_complementarity) + sum(box_complementarity) == gap,
            "Exact complementarity decomposition does not equal duality gap")
    report = dict(status="INDEPENDENT_PHASE1_GRAPH_MODEL_PRIMAL_DUAL_AUDIT_PASS",
                  ...
                  thresholded_kkt_values_are_diagnostics_only=True,
                  exact_maximum_row_complementarity=rational(max(row_complementarity)),
                  exact_maximum_box_complementarity=rational(max(box_complementarity)),
                  exact_complementarity_sum_equals_gap=True,
