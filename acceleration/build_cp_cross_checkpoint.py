"""Index independently audited cross-sign single3/4 CP search without redefining the goal."""
import argparse
...
def main():
    ...
    def read(name, status=None):
        ...
        if isinstance(data, dict) and data.get('status') == 'CP_CROSS_PREFLIGHT_AND_CORRUPTED_FAMILY_ASSOCIATION_CONTROLS_PASS':
            # This unit report embeds stdout from in-memory virtual fixtures.
            # Those path/hash strings describe mocked inputs, not archived
            # research files. Bind the whole report and its declared real
            # dependencies; never silently ignore a missing research artifact.
            require(data['positive_controls'] == 5 and data['negative_controls'] == 25
                    and data['gpu_launches'] == data['lp_solves'] == data['driver_output_directories_created'] == 0,
                    'Unexpected mocked preflight control scope')
            for source, expected in data['inputs_sha256'].items():
    ...
    family = read(args.family_audit, 'INDEPENDENT_COMPLETE_CROSS_ATOMIC_CYCLE_EXTRACTION_PASS')
    require(family['raw_cycles'] == 23870 and len(family['by_class']) == 14
            and all(r['matching_class'] == 'cross' and r['cycle_size'] in (3, 4) for r in family['by_class']),
            'Wrong cross single-cycle coverage')
    directory = args.directory.resolve()
    search = read(directory/'summary.json', 'BOUNDED_CP_MATCHING_SEARCH_FINISHED')
    audit = read(directory/'audit.json', 'INDEPENDENT_CP_CROSS_SEARCH_AUDIT_PASS')
    ...
    result = dict(status='HASH_VERIFIED_CP_CROSS_CONTINUATION_CHECKPOINT',
                  ...
                  independently_complete_cross_single_3_4_cycle_finals=family['legal_count'],
                  cross_scope='One cross-sign coordinate single alternating cycle of3or4 edges; neither full cross perfect matchings nor joint-coordinate changes.',
                  ...
                  embedded_mock_preflight_stdout_is_test_data_not_physical_research_files=True,
