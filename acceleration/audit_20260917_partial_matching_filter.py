...
ROOT=scope.ROOT
D=ROOT/'acceleration/results/20260917_partial_matching_filter'
OUT=ROOT/'acceleration/results/20260917_independent_review/partial_matching_filter'
...
def main():
    ...
    proofpath=ROOT/'acceleration/results/20260917_independent_review/partial_matching/summary.json'
    scope.require(scope.digest(proofpath)=='ed1d5b464466562cff722e45593095ec89c63bda761cfa7f784994abfc308b17','domain audit pin')
    proof=read(proofpath);scope.require(proof['status']=='INDEPENDENT_PARTIAL_K_DOMAINS_AND_LINEAR_CAPS_PASS','domain review')
    ...
    unknown={(a+15,b+15)for a,b in caps['variable_edges']}
    scope.require(len(unknown)==1740 and graphcheck.cap_ok(B),'partial graph')
    ctrl=controls();results=[]
    scope.require([v['outer_vertex']for v in summary['vertices']]==list(range(84)),'vertex population')
    for u in tqdm(range(84),desc='Independent partial matching filter',unit='center'):
        out=OUT/f'vertex_{u:02d}.json';p=D/f'vertex_{u:02d}.json';raw=read(p)
        masks=read(scope.D/f'domain_{u:02d}.json')['domain_masks_hex']
        ...
        scope.require(rejected==raw['rejected_ids'],'rejected ID partition')
