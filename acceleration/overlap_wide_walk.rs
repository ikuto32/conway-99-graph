//! Wide E0=0 overlap walk: unlike overlap_cpu.rs, block totals may change.
//! Derived from the frozen exact CPU kernel; its file and executable are untouched.
//! Build: rustc -O -C target-cpu=native acceleration/overlap_wide_walk.rs -o acceleration/overlap_wide_walk.exe
...
fn run_score(args: &[String], geometry: &Geometry) -> Result<()> {
    if args.len() != 5 && args.len() != 6 {
        return Err(
            "usage: overlap_wide_walk score CUTS.txt CANDIDATES.txt OUTPUT.json [repeats]".into(),
        );
    }
...
impl Counters {
    fn json(&self) -> String {
        format!("{{\"disjoint_edge_pairs\":{},\"overlap_and_absence_pass\":{},\"block_total_restriction_skipped\":{},\"label_quota_pass\":{},\"all_partial_caps_pass\":{}}}",
...
fn run_walk(args: &[String], geometry: &Geometry) -> Result<()> {
    if args.len() != 6 {
        return Err("usage: overlap_wide_walk walk CANDIDATES.txt OUTPUT.json steps seed".into());
    }
    ...
    output.push_str("],\"overlap_block_totals_fixed\":false,\"scope\":\"Bounded seeded E0=0 local walk with VARIABLE overlap block totals. Preserves overlap-only edges, degree four, exact own-root label quotas and every partial pair cap. Same-fibre edges remain absent. No disjoint completion or SRG certificate.\"}\n");
...
fn main() {
    ...
    let result = match args.get(1).map(String::as_str) {
        ...
        _ => Err("usage: overlap_wide_walk score CUTS.txt CANDIDATES.txt OUTPUT.json [repeats]\n       overlap_wide_walk walk CANDIDATES.txt OUTPUT.json steps seed".into()),
    };
