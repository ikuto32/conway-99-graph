"""Global-defect search retaining complete-star pair arc consistency.
...
consistency in the native checker. These are necessary conditions only.
...
def main():
    ...
    manifest = dict(inputs_sha256=bindings, seed=args.seed, iterations=args.iterations,
                    ...
                    scope='Finite global-defect guided K/X search retaining native complete-star pair arc consistency. Caps mean unavailable local checks, not exclusions. No full graph or exhaustive coverage.')
    ...
    write_json(args.out/'best_candidate.json', {'overlap_edges_outer_zero_based': best['candidate'],
               ...
               'source_manifest': path_key(args.out/'manifest.json')})
    write_json(args.out/'best_phase1.json', best['data'])
    summary = dict(status='BOUNDED_COUPLED_GLOBAL_DEFECT_SEARCH_FINISHED', stop_reason=stop,
