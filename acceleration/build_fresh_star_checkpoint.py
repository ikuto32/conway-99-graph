"""Index an audited fresh-star GPU cohort without implying a CP round ran."""
import argparse
...
EVALUATOR_SHA='affa5acf237e46b4c82c0ce867c56bb4ec6336d367dde2cd79ab3b23872d9ae5'
RANK_AUDITOR_SHA='93bbbc42386802471e9e8ab793ac30194f86d235ac489bfdd78d80e94c07f491'


def rebuild_selection(audit,count,mode):
    require(type(count) is int and count in (8,16) and mode in ('upper','union'),'Invalid cohort selection limits')
    all_indices=[r['proposal_index'] for r in audit['records']]
    require(len(all_indices)==len(set(all_indices)) and all(type(i) is int and i>=0 for i in all_indices),'Repeated or malformed rank attempt')
    require(all(type(r['available']) is bool for r in audit['records']),'Malformed ranking availability')
    available=[r for r in audit['records'] if r['available']]
    require(all(type(r[k]) in (int,float) and isfinite(r[k]) for r in available for k in ('best_upper_numeric','best_lower_numeric')),
    ...
    require(len(controls)==min(3,len(available)) and len({r['proposal_index'] for r in controls})==len(controls) and
        all(r['proposal_index'] in upper for r in controls),'Numerical CPU replay coverage required')
    require(len(set(upper))==len(upper) and upper==audit['ranked_by_upper_indices'] and lower==audit['ranked_by_lower_indices'],
...
def build(args):
    ...
    book.bind(ROOT/'acceleration/evaluate_fresh_star_shortlist.py',EVALUATOR_SHA)
    book.bind(ROOT/'acceleration/audit_fresh_star_ranking.py',RANK_AUDITOR_SHA)
    ...
    summary=book.read(args.evaluation,'BOUNDED_FRESH_STAR_SHORTLIST_EVALUATION_FINISHED')
    manifest=book.read(summary['manifest_path'],'FRESH_STAR_SHORTLIST_EVALUATION_MANIFEST')
    ...
    book.assert_bound(manifest,ROOT/'acceleration/evaluate_fresh_star_shortlist.py')
    ranking=book.read(manifest['ranking_path']);audit=book.read(manifest['ranking_audit_path'],'INDEPENDENT_FRESH_STAR_PDHG_RANKING_AUDIT_PASS')
    book.assert_bound(audit,manifest['ranking_path']);book.assert_bound(audit,ROOT/'acceleration/audit_fresh_star_ranking.py')
    ...
    require(audit['producer_or_native_imported'] is False and audit['all_serialized_models_reconstructed'] is True and
        audit['independent_domain_enumeration_performed'] is False,'Wrong independent ranking scope')
    ...
    require(family_audit['status'] in ('INDEPENDENT_COMPLETE_CROSS_ATOMIC_CYCLE_EXTRACTION_PASS','INDEPENDENT_COMPLETE_WHOLE_SAME_SIGN_MATCHING_FAMILY_PASS') and
    ...
    return dict(status='HASH_VERIFIED_STAR_GUIDED_ROUND_CHECKPOINT',created_utc=datetime.now(timezone.utc).isoformat(),
        fresh_star_GPU_cohort=True,CP_round_claimed=False,goal=previous['goal'],current_best=previous['current_best'],current_star_marginal_best=frontier,
