...
def main():
 ...
 export=json.loads((ROOT/EXPORT).read_bytes());bindings={GATE:GATE_SHA,EXPORT:digest(ROOT/EXPORT)}
 ...
 for path in (Path(__file__),ROOT/'acceleration/certify_20260917_six_moment_pdhg.py',ROOT/PROTOCOL):bindings[path.relative_to(ROOT).as_posix()]=digest(path)
