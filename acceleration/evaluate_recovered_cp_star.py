"""Evaluate a explicitly repaired and independently audited CP record view.
...
def select_records(summary, audit):
    ...
    require(summary['status'] == 'AUDITED_REPAIRED_CP_LP_RECORD_VIEW' and audit['status'] ==
            'INDEPENDENT_CP_MATCHING_REPAIRED_LP_AUDIT_PASS',
...
def preflight(args):
    ...
    require(key(audit['effective_summary_path'])==key(run/'summary.json') and
            audit['effective_summary_sha256']==bind(run/'summary.json') and
            key(summary['original_summary_path'])==key(audit['original_summary_path']) and
            summary['original_summary_sha256']==audit['original_summary_sha256'] and
            key(summary['repair_manifest_path'])==key(audit['repair_manifest_path']) and
            summary['repair_manifest_sha256']==audit['repair_manifest_sha256'] and
            summary['audit_tolerance_changed'] is False and audit['independent_LP_audit_tolerance']==1e-7,
            'Recovered view/original/repair association differs')
    audit_inputs=normalized(audit['inputs_sha256'])
    require(audit_inputs.get(key(ROOT/'acceleration/audit_cp_matching_recovery.py'))==PINS['audit_cp_matching_recovery.py'],
            'Recovery audit implementation differs')
    for name,expected in ((summary['original_summary_path'],summary['original_summary_sha256']),
                          (summary['repair_manifest_path'],summary['repair_manifest_sha256'])):
        require(audit_inputs.get(key(name))==bind(name,expected),'Unbound recovered view origin')
    ...
    return dict(status='RECOVERED_CP_STAR_SHORTLIST_EVALUATION_MANIFEST', inputs_sha256=inputs,
        recovery_view=True,original_search_preserved=True,independent_LP_audit_tolerance=1e-7,
