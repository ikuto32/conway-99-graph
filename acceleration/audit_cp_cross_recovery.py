"""Independently recover a cross3/4 CP audit using explicit higher-accuracy LP replacements.
...
PRECISE_SOURCE_SHA = '7b77d2b4f201706d7ec5fe06d7729e34cf9d9180d7b15d7411c939992ef491ad'
...
def main():
    ...
    parser.add_argument("--view-out", type=Path, required=True)
    parser.add_argument("--lp-repairs", type=Path, required=True)
    ...
    require(not args.view_out.exists() and args.view_out.resolve()!=args.out.resolve(), 'Use two fresh recovery outputs')
    ...
    repairs_document = load(args.lp_repairs)
    require(repairs_document['status']=='EXPLICIT_HIGH_ACCURACY_CP_LP_REPLACEMENTS' and
            resolve(repairs_document['original_summary_path']).resolve()==(args.run/'summary.json').resolve() and
            repairs_document['original_summary_sha256']==bind(args.run/'summary.json') and
            repairs_document['original_files_overwritten'] is False and repairs_document['audit_tolerance_changed'] is False,
            'Unbound or unsupported LP repair manifest')
    for p,h in repairs_document['inputs_sha256'].items(): bind(p,h)
    repair_rows=repairs_document['records']
    repairs={r['proposal_index']:r for r in repair_rows}
    require(0<len(repairs)==len(repair_rows) and all(type(i) is int for i in repairs), 'Invalid/repeated repair indices')
    require(set(repairs)<=set(r['proposal_index'] for r in summary['records']), 'Repair outside selected CP inventory')
    effective_records=[]
    ...
    require(summary["producer_version"] == manifest["producer_version"] == "cross1", "Expected cross1 driver")
    ...
    for field in ("native", "family_audit", "initial", "initial_phase1", "gpu_audit", "gpu", "gpu_source"):
        ...
        require(supplied is None or supplied.resolve() == declared.resolve(), "Supplied path differs from cross1 manifest: " + field)
    ...
    for path in (args.native, args.family_audit, args.initial, args.initial_phase1, args.gpu_audit, args.gpu, args.gpu_source,
                 ROOT / "acceleration/search_cp_cross.py"):
    ...
    require(family["status"] == "INDEPENDENT_COMPLETE_CROSS_ATOMIC_CYCLE_EXTRACTION_PASS" and
            family["all_original_native_indices_and_final_graphs_checked"] is True and
            family["all_cycles_min_vertex_removed_first_checked"] is True and
            family["producer_imported"] is False, "Wrong or incomplete cross extraction proof")
    ...
    require(native["status"] == "COMPLETE_CROSS_ATOMIC_CYCLE_SUBFAMILY_EXTRACTION" and
            native["selector"] == "cross_3_4" and native["cycle_sizes"] == [3, 4], "Wrong native family scope")
    for path, expected_sha in native["inputs_sha256"].items():
        bind(path, expected_sha)
    for pathfield, hashfield in (("candidate_path", "candidate_sha256"),
                                ("source_native_path", "source_native_sha256"),
                                ("source_family_audit_path", "source_family_audit_sha256")):
        p = resolve(native[pathfield])
        require(family_bindings.get(p.resolve()) == bind(p, native[hashfield]), "Unbound atomic extraction provenance")
    require(signature(load(native["candidate_path"])["overlap_edges_outer_zero_based"]) == initial_sig,
            "Atomic extraction was made at a different base graph")
    full_family_proof = load(native["source_family_audit_path"])
    require(full_family_proof["status"] == "INDEPENDENT_COMPLETE_ATOMIC_CYCLE_FAMILY_PASS" and
            full_family_proof["cycle_size"] == "both", "Underlying complete atomic3/4 proof missing")
    require(all(type(move["root_group"]) is int and 0 <= move["root_group"] < 7 and
                move["matching_class"] == "cross" and type(move["cycle_size"]) is int and move["cycle_size"] in (3, 4) and
                move["changed_edges"] == move["cycle_size"] and
                len(move["removed"]) == len(move["added"]) == move["cycle_size"] and
                partition(move) == (move["cycle_size"],) and
                move["alternating_cycles"] == [move["alternating_cycle"]]
                for move in moves), "Adapter move is outside the single cross3/4 family")
    cross_size_counts = Counter(move["cycle_size"] for move in moves)
    require({str(k): v for k, v in cross_size_counts.items()} == family["per_cycle_size_legal"], "Cross size counts differ")
    class_counts = Counter((move["root_group"], move["cycle_size"]) for move in moves)
    cross_classes = [row for row in full_family_proof["by_class"] if row["matching_class"] == "cross"]
    require(native["by_class"] == family["by_class"] == cross_classes and len(cross_classes) == 14,
            "Cross coordinate metadata differ from original atomic proof")
    require(all(row["legal_cycles"] == class_counts[row["root_group"], row["cycle_size"]]
                for row in cross_classes), "Cross coordinate final count differs")
    require(native["raw_cycles"] == family["raw_cycles"] == sum(row["raw_cycles"] for row in cross_classes) == 23870 and
            native["unchanged_count"] == family["unchanged_count"] == 0 and
            native["legal_count"] == family["legal_count"] == len(keys), "Cross count/scope differs")
    for native_field, row_field in (("support_rejected_count", "support_rejected"), ("cap_rejected_count", "cap_rejected")):
        require(native[native_field] == family[native_field] == sum(row[row_field] for row in cross_classes), "Cross rejection count differs")
    require(native["raw_cycles"] == len(keys) + native["support_rejected_count"] + native["cap_rejected_count"],
            "Cross family count partition failed")
    require(native["original_native_indices"] == [move["original_native_index"] for move in moves] and
            len(set(native["original_native_indices"])) == len(keys), "Original atomic index associations differ")
    del options
    stage_reports = []
    ...
    require(all(type(v) is int for v in (coarse_steps, refine_steps, refine_count, lp_count)) and
            ...
            140 <= refine_count <= min(99999, len(keys)) and lp_count <= refine_count, "Invalid finite cross1 study size")
    ...
    for chosen, record in zip(expected, summary["records"]):
        ...
        if i in repairs:
            replacement=repairs[i]
            require(resolve(replacement['candidate_path']).resolve()==candidate_path.resolve() and
                    replacement['candidate_sha256']==bind(candidate_path) and
                    resolve(replacement['original_result_path']).resolve()==result_path.resolve() and
                    replacement['original_result_sha256']==bind(result_path), 'LP repair candidate/original association differs')
            try:
                inspect_artifact(candidate_path, result_path)
            except ValueError as error:
                require(str(error)==replacement['original_strict_audit_error']=='Phase-I primal-dual gap too large',
                        'Repair hides another strict-audit failure')
            else:
                raise ValueError('Unnecessary replacement of an already-passing LP')
            result_path=resolve(replacement['replacement_result_path'])
            require(result_path.resolve()!=original_result_path.resolve(), 'Replacement overwrote original LP')
            bind(result_path,replacement['replacement_result_sha256'])
            effective_result=load(result_path)
            require(resolve(effective_result['candidate_path']).resolve()==candidate_path.resolve() and
                    effective_result['candidate_sha256']==bind(candidate_path) and
                    effective_result['overlap_edges_sha256']==edge_hash(keys[i]) and effective_result['optimal'] is True,
                    'Replacement LP is for another graph or is incomplete')
            require(effective_result['source_sha256'].get('phase1_probe_precise.py')==PRECISE_SOURCE_SHA and
                    effective_result['requested_solver_tolerances']==dict(ipm_optimality_tolerance=1e-10,
                        primal_feasibility_tolerance=1e-10,dual_feasibility_tolerance=1e-10) and
                    effective_result['independent_audit_tolerance_changed'] is False, 'Replacement precision policy differs')
            for name,expected_sha in effective_result['source_sha256'].items():
                require(Path(name).name==name,'Bad replacement source path')
                bind(ROOT/'acceleration'/name,expected_sha)
        interval = inspect_artifact(candidate_path, result_path)
        if i in repairs:
            repair_audit=load(repairs[i]['strict_audit_path'])
            bind(repairs[i]['strict_audit_path'],repairs[i]['strict_audit_sha256'])
            require(repair_audit['status']=='INDEPENDENT_PHASE1_GRAPH_MODEL_PRIMAL_DUAL_AUDIT_PASS' and
                    repair_audit['numerical_tolerance']==1e-7 and
                    {resolve(p).resolve():h for p,h in repair_audit['inputs_sha256'].items()}==
                    {resolve(p).resolve():h for p,h in interval['inputs_sha256'].items()} and
                    all(repair_audit[k]==interval[k] for k in ('exact_primal_upper_bound','exact_dual_lower_bound',
                        'exact_primal_dual_gap','auditor_sha256','graph_auditor_sha256')),
                    'Stored strict replacement audit differs from fresh replay')
        effective_record=deepcopy(record)
        effective_record.update(result_path=relative(result_path),result_sha256=bind(result_path),
            numeric_objective=effective_result['numeric_objective'],optimal=effective_result['optimal'],status=effective_result['status'])
        ...
        reports.append(dict(proposal_index=i, candidate_path=record["candidate_path"], result_path=relative(result_path),
                            original_result_path=relative(original_result_path),original_result_sha256=bind(original_result_path),
                            LP_replaced=i in repairs, numeric_objective=effective_result['numeric_objective'], phase1_audit=interval,
    ...
    view.update(status='AUDITED_REPAIRED_CP_LP_RECORD_VIEW', records=effective_records,
        original_summary_path=relative(args.run/'summary.json'),original_summary_sha256=bind(args.run/'summary.json'),
        repair_manifest_path=relative(args.lp_repairs),repair_manifest_sha256=bind(args.lp_repairs),
        original_search_preserved=True,original_audit_failure_preserved=True,audit_tolerance_changed=False,
        repaired_proposal_indices=sorted(repairs),inputs_sha256=dict(hashes))
    effective_order=sorted(effective_records,key=lambda r:(r['numeric_objective'],r['proposal_index']))
    effective_improvements=[r for r in effective_order if r['numeric_objective']<warm['numeric_objective']]
    view.update(best=effective_order[0],numerically_improving_candidates=effective_improvements,
        numerical_improvement_count=len(effective_improvements))
    view['outputs_sha256']=dict(summary['outputs_sha256'])
    for row in effective_records:
        view['outputs_sha256'][row['result_path']]=row['result_sha256']
    args.view_out.parent.mkdir(parents=True,exist_ok=True)
    with args.view_out.open('x',encoding='utf-8') as stream: stream.write(json.dumps(view,indent=2)+'\n')
    bind(args.view_out)
    report = dict(status="INDEPENDENT_CP_CROSS_REPAIRED_LP_AUDIT_PASS", audited_producer_version="cross1", inputs_sha256=hashes,
                  ...
                  effective_summary_path=relative(args.view_out),effective_summary_sha256=bind(args.view_out),
                  original_summary_path=relative(args.run/'summary.json'),original_summary_sha256=bind(args.run/'summary.json'),
                  repair_manifest_path=relative(args.lp_repairs),repair_manifest_sha256=bind(args.lp_repairs),
                  repaired_proposal_indices=sorted(repairs),original_search_and_failed_LP_files_preserved=True,
                  independent_LP_audit_tolerance=1e-7,LP_solutions_replaced_from_bound_external_artifacts=len(repairs),
                  ...
                  effective_best_numeric_proposal_index=effective_order[0]['proposal_index'],
                  effective_best_numeric_objective=effective_order[0]['numeric_objective'],
