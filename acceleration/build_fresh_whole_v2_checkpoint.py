"""Index audited whole fresh v2 shortlist and independently reviewed exact scope.
...
EVALUATOR_SHA='b4dd20df4c0c2b86ad2e0c866d497442ea40e415f396474c7660f9391e95ca9e'
RANK_AUDITOR_SHA='580e2c4eeba8d50b18c8f207e6985f0ba318914d5d6cd69edc8e439c5569efd2'
...
def check_whole_identity(ranking,audit,family,manifest):
    require(ranking['status']=='NUMERICAL_WHOLE_FRESH_STAR_PDHG_RANKING_FINISHED' and
        ranking['producer_version']=='whole_fresh_v2_balanced' and
    ...
    for numerical,checked in zip(ranking['records'],audit['records']):
        ...
        require((numerical['status']=='NUMERICALLY_SCORED')==checked['available'],'Ranking availability differs')
...
def check_independent_review(book,args,summary):
    ...
    scope=book.read(args.scope_review,'INDEPENDENT_WHOLE_FRESH_STAR_SHORTLIST_AUDIT_PASS')
...
def build(args):
    ...
    book.bind(ROOT/'acceleration/evaluate_fresh_whole_v2.py',EVALUATOR_SHA)
    book.bind(ROOT/'acceleration/audit_20260917_whole_fresh_ranking.py',RANK_AUDITOR_SHA)
    ...
    summary=book.read(args.evaluation,'BOUNDED_WHOLE_FRESH_STAR_SHORTLIST_EVALUATION_FINISHED')
    scope_review=check_independent_review(book,args,summary)
    manifest=book.read(summary['manifest_path'],'WHOLE_FRESH_STAR_SHORTLIST_EVALUATION_MANIFEST_V2')
    ...
    book.assert_bound(manifest,ROOT/'acceleration/evaluate_fresh_whole_v2.py')
    ranking=book.read(manifest['ranking_path']);audit=book.read(manifest['ranking_audit_path'],'INDEPENDENT_WHOLE_FRESH_STAR_PDHG_RANKING_AUDIT_PASS')
    book.assert_bound(audit,manifest['ranking_path']);book.assert_bound(audit,ROOT/'acceleration/audit_20260917_whole_fresh_ranking.py')
    ...
    return dict(status='HASH_VERIFIED_STAR_GUIDED_ROUND_CHECKPOINT',created_utc=datetime.now(timezone.utc).isoformat(),
        fresh_star_GPU_cohort=True,whole_fresh_v2=True,independent_scope_review_path=key(args.scope_review),
