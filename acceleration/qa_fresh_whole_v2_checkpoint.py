...
def main():
    ...
    records=[dict(proposal_index=i,available=True,best_upper_numeric=float(i+20),best_lower_numeric=float(32-i)) for i in range(32)]
    upper=list(range(32));lower=list(reversed(upper))
    audit=dict(records=records,ranked_by_upper_indices=upper,ranked_by_lower_indices=lower,
        numeric_cpu_controls=[dict(proposal_index=i) for i in (0,16,31)],
        evaluation_selections={'union_16':[dict(proposal_index=i,selection_roles=['upper' if i<8 else 'lower']) for i in list(range(8))+list(range(31,23,-1))]})
    ...
    from qa_evaluate_fresh_whole_v2 import fixtures
    ...
    book.bind(b.ROOT/'acceleration/qa_evaluate_fresh_whole_v2.py')
    for name,h in b.PINS.items():book.bind(b.ROOT/'acceleration'/name,h)
    book.bind(Path(__file__));book.bind(Path(b.__file__))
    report=dict(status='WHOLE_FRESH_V2_CHECKPOINT_CONTROLS_PASS',inputs_sha256=book.verified,
