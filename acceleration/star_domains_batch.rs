//! Batch derivative of star_domains.rs; original source remains frozen.
//! Derivation parent SHA256: b3637615352c0a5372fe7ed2237f34bc2d5c8773a892ee647c05acb772584d80
//! rustc -O -C target-cpu=native acceleration/star_domains_batch.rs -o acceleration/build/star_domains_batch.exe
//! star_domains_batch INPUT.txt OUTPUT.json [seconds_per_candidate=30] [node_cap=2000000] [domain_cap=20000]
//! INPUT uses C99OVERLAPS1; output results preserve candidate input order.
//! Each candidate has fresh enumeration budgets. A cap never implies exclusion.
...
fn run() -> Result<()> {
    ...
    if !(3..=6).contains(&args.len()) {
        return Err(
            "usage: star_domains_batch INPUT.txt OUTPUT.json [seconds_per_candidate] [node_cap] [domain_cap]".into(),
        );
    }
    ...
    if tokens.next() != Some("C99OVERLAPS1") {
        return Err("expected C99OVERLAPS1 header".into());
    }
    ...
    for index in 0..count {
        candidates.push(
            Graph::read_candidate(&mut tokens)
                .map_err(|message| format!("candidate {index}: {message}"))?,
        );
    }
    ...
    for (index, candidate) in candidates.iter().enumerate() {
        ...
        output.push_str(&evaluate_candidate(
            candidate, seconds, node_cap, domain_cap,
        )?);
    }
    let elapsed = start.elapsed().as_secs_f64();
    write!(&mut output, "],\"candidate_count\":{count},\"caps_reset_per_candidate\":true,\"elapsed_seconds\":{elapsed:.9}}}\n").unwrap();
    fs::write(&args[2], output).map_err(|e| e.to_string())?;
    eprintln!("{count} candidates in {elapsed:.6} seconds; enumeration caps reset per candidate");
    Ok(())
}
