"""Bind fresh whole-family exact certificates to independently selected graphs."""
import argparse
...
def main():
    ...
    raw.check(rank['status']=='INDEPENDENT_WHOLE_FRESH_STAR_PDHG_RANKING_AUDIT_PASS','ranking gate')
    raw.check(bind(m['ranking_audit_path'],m['ranking_audit_sha256'])==
              '34f72e00cf1b6e734b6be706c12ce414d8978a6f24803e648eefbc0d19bb8788','frozen independent ranking')
    avail=[r for r in rank['records'] if r['available']]
    ...
    claims=[dict(id='C-WHOLE-FRESH-STAR-16-EXCLUSIONS',revision=1,recommendation='VERIFIED',scope='Exactly these16 labeled fixedK assignments with prescribed absent edges; positive independent exact original-star lower bounds',indices=selected)]
    if worse==selected:
        claims.append(dict(id='C-WHOLE-FRESH-STAR-16-NO-IMPROVEMENT',revision=1,recommendation='VERIFIED',scope='All16 exact original-star lower bounds exceed incumbent18481 upper bound',indices=selected,dependencies=[dict(id='C-STAR-BASELINE-18481',revision=1,relation='uses_result')]))
    raw.check(all(raw.digest(f)==h for f,h in bindings.items()),'bound inputs changed')
    report=dict(status='INDEPENDENT_WHOLE_FRESH_STAR_SHORTLIST_AUDIT_PASS',timestamp=datetime.now(timezone.utc).isoformat(),
        ...
        claim_bindings=claims,selected_indices=selected,distinct_selected_graphs=16,independent_raw_passes=16,original_domains_per_graph=84,domain_choices_checked=domain_count,
