...
ROOT=h.ROOT;D=ROOT/'acceleration/results/20260917_four_matching_moments';TABLES=ROOT/'acceleration/results/20260917_partial_four_matchings'
def main():
    ...
    h.require(n==290460 and len(fixed)==144 and len(unknown)==1920 and meta['supports']==list(map(list,labels))and meta['fixed_edges']==sorted(map(list,fixed))and meta['unknown_edges']==list(map(list,unknown))and meta['pair_order']==list(map(list,pairs))and meta['probability_offsets']==offsets,'exact model indexing')
    ...
    expected=csc_matrix((np.asarray(values,dtype=np.int64),np.asarray(rows),np.asarray(ptr)),shape=(5490,297432)).tocsr();expected.sort_indices()
    ...
    h.require(meta['rhs']==rhs and meta['costs']==[0]*n+[1]*6972 and meta['column_lower']==0 and meta['column_upper']is None and meta['all_rows_equalities']is True and meta['nonzeros']==actual.nnz==24085997,'full objective/bounds/rhs')
    ...
    report=dict(status='INDEPENDENT_FOUR_COORDINATE_FULL_MOMENT_MODEL_PASS',claim_id='C-PARTIAL-K-FOUR-COORDINATE-FULL-MOMENT-ENCODING',claim_revision=1,recommendation='VERIFIED',statement='Every SRG completion in the frozen four-coordinate partial-K scope induces a zero-objective feasible point in this exact full-neighborhood moment LP; no converse claimed.',timestamp=datetime.now(timezone.utc).isoformat(),source_commit=head,command=[sys.executable,*sys.argv],cwd=str(ROOT),python=platform.python_version(),numpy=np.__version__,scipy=scipy.__version__,inputs_sha256=bindings,objective=meta['model'],shape=[5490,297432],nonzeros=24085997,probability_columns=n,hard_simplex_rows=84,hard_reciprocity_rows=1920,soft_moment_equalities=3486,complete_augmented_matrix_and_bounds_checked=True,chunk_identity_checked=True,reconstructed_chunk_bytes=length,chunk_count=len(artifact['parts']),controls=controls,producer_imported=False,dependency=dict(id='C-PARTIAL-K-FOUR-COORDINATE-DOMAINS',revision=1,relation='uses_result'),scope=primary['scope'],derivation='A completion chooses exactly one admissible local star at each outer center. Reciprocity equates two representations of each unknown edge. Pair moments count all common outer neighbors through full center neighborhoods; adding the smaller-endpoint unknown edge marginal gives common_outer+A_unknown = 2-common_inner-B_fixed. Consequently all moment slacks can vanish. The converse is not needed.',shared_components=['Python standard library','NumPy/SciPy exact integer sparse storage','prior independent full99/domain artifact helpers'],limitations=['Necessary relaxation only; no new four-coordinate feasibility or exclusion certificate.','All290460 original new domains used; no matching or pair filter.','No unrestricted target resolution.'])
    out=ROOT/'acceleration/results/20260917_independent_review/four_matching_moments.json'
