"""Original-star evaluation of independently audited v3 reranking untested union16.
...
PINS['evaluate_fresh_whole_v2.py']='b4dd20df4c0c2b86ad2e0c866d497442ea40e415f396474c7660f9391e95ca9e'
...
RANKING_AUDIT_STATUS='INDEPENDENT_WHOLE_STAR_RERANK_V3_AUDIT_PASS'

def select_records(audit,family,count,mode):
    require(audit['status']==RANKING_AUDIT_STATUS and audit['producer_or_native_imported'] is False and
        audit['original_serialized_models_independently_verified_via_reuse'] is True and
        audit['new_model_reconstruction_performed'] is False and audit['independent_domain_enumeration_performed'] is False,
        'Fresh independent reranking audit with honest model reuse scope required')
    require(type(count) is int and count==16 and mode=='union','Frozen v3 union16 selection required')
    ...
    n=family['legal_count'];require(n==len(family['overlap_candidates'])==len(family['moves']),'Incomplete family inventory')
    rows=audit['records'];indices=[r['proposal_index'] for r in rows]
    require(len(indices)==len(set(indices))==128 and all(type(i) is int and 0<=i<n for i in indices),'Exactly128 reranking records required')
    require(all(type(r.get('original_native_index')) is int and r['original_native_index']==r['proposal_index'] for r in rows),
        'Whole native identity differs')
    excluded=audit['excluded_LP_tested_indices']
    require(len(excluded)==len(set(excluded))==16 and all(type(i) is int for i in excluded) and set(excluded)<=set(indices),
        'Exactly16 prior LP tested IDs required')
    require(all(r['available'] is True for r in rows),'Full128 reranking required before shortlist')
    require(all(type(r[k]) in (int,float) and isfinite(r[k]) for r in rows for k in ('best_upper_numeric','best_lower_numeric')),
        'Nonfinite reranking scores')
    require(all(r['best_upper_numeric']>=-1e-10 and r['best_lower_numeric']<=r['best_upper_numeric']+1e-7 for r in rows),
        'Inconsistent numerical interval')
    eligible=[r for r in rows if r['proposal_index'] not in excluded];require(len(eligible)==112,'Eligible population is112')
    ...
    require(len(controls)==3 and len({r['proposal_index'] for r in controls})==3 and
        all(r['proposal_index'] in indices for r in controls),'Three independent numeric CPU controls required')
    require(upper==audit['eligible_ranked_by_upper_indices'] and lower==audit['eligible_ranked_by_lower_indices'],
        'Independent eligible112 ranking order differs')
    chosen=evaluation_selection(upper,lower,16,'union')
    require(chosen==audit['evaluation_selections']['union_16'] and len(chosen)==16 and
        not set(excluded)&{r['proposal_index'] for r in chosen},'Independent untested union16 differs')
    return chosen,upper,lower,[]


def verify_whole_records(ranking,audit,family):
    require(ranking['status']=='NUMERICAL_WHOLE_STAR_RERANK_V3_FINISHED' and
        ranking['iterations']==5000 and ranking['source_iterations']==500 and ranking['objective_id']==OBJECTIVE,
    ...
    for numerical,checked in zip(ranking['records'],audit['records']):
        ...
        require((numerical['status']=='NUMERICALLY_RERANKED')==checked['available'],'Availability differs')
...
def preflight(args):
    ...
    require(type(args.max_candidates) is int and args.max_candidates==16 and args.selection=='union' and
        isfinite(args.seconds) and args.seconds==30 and isfinite(args.audit_seconds) and args.audit_seconds==60,
    ...
    require(ranking['status']=='NUMERICAL_WHOLE_STAR_RERANK_V3_FINISHED' and ranking['iterations']==5000 and ranking['source_iterations']==500 and ranking['objective_id']==OBJECTIVE,'Whole v3 reranking/objective required')
    ...
    original_model_audit=read(audit['original_model_audit_path'])
    bind(audit['original_model_audit_path'],audit['original_model_audit_sha256'])
    payload_audit=read(audit['payload_identity_review_path'])
    bind(audit['payload_identity_review_path'],audit['payload_identity_review_sha256'])
    for field in ('original_model_audit','payload_identity_review'):
        require(audit_inputs.get(key(audit[field+'_path']))==bind(audit[field+'_path']), 'Independent model reuse dependency is not bound')
    require(original_model_audit['status']=='INDEPENDENT_WHOLE_FRESH_STAR_PDHG_RANKING_AUDIT_PASS' and
        original_model_audit['all_serialized_models_reconstructed'] is True and original_model_audit['producer_or_native_imported'] is False and
        payload_audit['status']=='INDEPENDENT_WHOLE_STAR_RERANK_V3_INPUT_PASS' and payload_audit['producer_imported'] is False,
        'Original model reconstruction and full payload identity audits required')
    require(original_model_audit['input_selected_indices']==ranking['input_selected_indices']==payload_audit['selected_indices'] and
        ranking['excluded_LP_tested_indices']==audit['excluded_LP_tested_indices']==payload_audit['excluded_LP_tested_indices'],
        'Frozen reused model / excluded16 inventory differs')
    preparation=read(ranking['preparation_path']);bind(ranking['preparation_path'],ranking['preparation_sha256'])
    calibration_review=read(preparation['calibration_review_path']);bind(preparation['calibration_review_path'],preparation['calibration_review_sha256'])
    require(calibration_review['status']=='INDEPENDENT_WHOLE_FRESH_STAR_SHORTLIST_AUDIT_PASS' and
        calibration_review['selected_indices']==audit['excluded_LP_tested_indices'], 'Prior16 exact LP exclusion list is not independently bound')
    require(audit_inputs.get(key(ranking['preparation_path']))==bind(ranking['preparation_path']), 'Independent reranking audit lacks preparation binding')
    ...
    require(ranking['reranked_candidates']==128 and ranking['new_candidates']==0 and ranking['eligible_for_next_LP']==112 and
        ranking['unavailable_count']==0 and ranking['eligible_ranked_by_upper']==upper and ranking['eligible_ranked_by_lower']==lower and
        ranking['proposed_union16']==selection,'Producer reranking population/eligible union differs from independent audit')
    ...
    return dict(status='WHOLE_STAR_RERANK_SHORTLIST_EVALUATION_MANIFEST_V3',objective_id=OBJECTIVE,original_complete_domains_used=True,pair_pruned_domains_used=False,ranking_auditor_path=key(auditor_path),ranking_auditor_sha256=bind(auditor_path),inputs_sha256=inputs,
        ...
        reranked_existing_candidates=128,new_ranked_candidates=0,excluded_LP_tested_indices=audit['excluded_LP_tested_indices'],
        original_model_audit_path=key(audit['original_model_audit_path']),original_model_audit_sha256=bind(audit['original_model_audit_path']),
        payload_identity_review_path=key(audit['payload_identity_review_path']),payload_identity_review_sha256=bind(audit['payload_identity_review_path']),
...
def execute(args,manifest):
    ...
    try:
        ...
        save(out/'summary.json', dict(status='BOUNDED_WHOLE_STAR_RERANK_SHORTLIST_V3_EVALUATION_FINISHED',objective_id=OBJECTIVE, manifest_path=key(out/'manifest.json'),
        ...
        save(out/'failure.json', dict(status='WHOLE_STAR_RERANK_SHORTLIST_V3_EVALUATION_STOPPED', active_index=active,
...
def main():
    ...
    if args.validate_only:
        print(json.dumps(dict(status='WHOLE_STAR_RERANK_SHORTLIST_V3_READ_ONLY_PREFLIGHT_PASS',
            ...
            reranked_existing_candidates=128,new_ranked_candidates=0,already_LP_tested_excluded=16,
