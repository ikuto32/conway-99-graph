...
POLICY=dict(name='64_BEST_REFINED_UPPER_THEN64_BALANCED_COORDINATES_ROTATING_CYCLE_SHAPES_V2B',
    ...
    stratum='Outer14 coordinates(root_group,matching_class); inner sorted changed-edge counts of all alternating cycles',
    diverse_order='After removing global64, round-robin all14 sorted(root_group,matching_class) coordinates; each coordinate independently rotates sorted nonempty cycle-shape buckets, taking lowest(score,index) in that bucket; stop at64',
...
def choose(scores,moves,excluded):
    ...
    coordinates=sorted(buckets)
    require(coordinates==[(g,'same_'+str(s)) for g in range(7) for s in range(2)],'All14 coordinates must have eligible diversity members')
    shape_order={coordinate:sorted(buckets[coordinate]) for coordinate in coordinates}
    shape_cursor={coordinate:0 for coordinate in coordinates}
    while len(selected)<128:
        changed=False
        for coordinate in coordinates:
            shapes=shape_order[coordinate]
            for _ in range(len(shapes)):
                j=shape_cursor[coordinate];shape_cursor[coordinate]=(j+1)%len(shapes)
                bucket=buckets[coordinate][shapes[j]]
                if bucket:
                    i=bucket.popleft();selected.append(i);roles[i]=['balanced_coordinate_then_cycle_shape64'];changed=True
                    break
            if len(selected)==128:break
        require(changed,'Diversity coordinate exhaustion')
...
def execute(context):
    ...
    summary = dict(status='NUMERICAL_WHOLE_FRESH_STAR_PDHG_RANKING_FINISHED', producer_version='whole_fresh_v2_balanced', objective_id=OBJECTIVE,
