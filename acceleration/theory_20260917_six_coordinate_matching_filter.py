    ...
    for u in tqdm(range(len(completed),84),desc='Six-coordinate neighborhood matching',unit='center'):
        ...
        for i in range(len(records),len(table)):
            ...
            try:
                ...
                part_path=args.out/f'failed_partial_vertex_{u:02d}.json'
                save(part_path,dict(outer_vertex=u,original_count=len(table),records=records,complete=False))
                packed=companion(part_path)
                if packed:companions.append(packed)
                compressed_manifest=save_companions(args.out,companions)
                failure=dict(timestamp=stamp(),status='INCOMPLETE_CHECK_FAILURE',inputs_sha256=hashes,
                    outer_vertex=u,original_id=i,mask=table[i],exception_type=type(exc).__name__,exception=str(exc),
                    completed_centers=completed,partial_path=str(part_path),partial_sha256=digest(part_path),
                    compressed_manifest=compressed_manifest,fresh_evaluations=fresh_evaluations,
                    retry_authorized=False,target_resolution=False)
                save(args.out/'failure.json',failure)
                print(json.dumps({k:v for k,v in failure.items() if k not in('inputs_sha256','completed_centers')}))
                return 4
