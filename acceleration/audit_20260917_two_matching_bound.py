"""Direct-neighborhood integer support checks on the frozen two-coordinate family."""
import argparse
...
ROOT=h.ROOT;TABLES=ROOT/'acceleration/results/20260917_partial_two_matchings';MODEL=ROOT/'acceleration/results/20260917_two_matching_moments'
def main():
    ...
    if args.mode=='transfer':
        D=ROOT/'acceleration/results/20260917_two_matching_transferred_certificate';read(D/'manifest.json');summary=read(D/'summary.json');cert=read(D/'certificate.json');h.require(summary['certificate_sha256']==h.digest(D/'certificate.json'),'transfer certificate binding')
    else:
    ...
    h.require(type(scale)is int and scale>0 and len(qv)==1800 and len(yv)==3486 and all(type(v)is int for v in qv+yv)and all(abs(v)<=scale for v in yv),'weight domain')
    y=dict(zip(pairs,yv));q=dict(zip(unknown,qv))
    if args.mode=='transfer':
        old=read(ROOT/'acceleration/results/20260917_partial_matching_moment_replay600/exact_support_bound.json')['bound'];oldcaps=read(h.D/'linear_caps.json');oldq=dict(zip(map(tuple,oldcaps['variable_edges']),old['reciprocity_weight_numerators']))
        ...
        newedges=[list(e)for e in unknown if e not in oldq];h.require(newedges==cert['new_zero_weight_edges']and len(newedges)==60 and cert['transferred_reciprocity_edges']==1740,'transfer scope')
    ...
    num=dot-sum(maxima);h.require(count==89308 and maxima==cert['center_maxima_numerators']and num==cert['numerator']and (num>0)==cert['strictly_positive'],'exact full support result')
    if args.mode=='transfer':h.require(dot==cert['rhs_dot_numerator']and argmax==cert['maximizing_new_domain_ids'],'transfer maxima identities')
