"""Bounded complete-domain pilot for six freed same-sign coordinates; no LP."""
import argparse
...
OLD=Path('acceleration/results/20260917_partial_four_matchings')
AUDIT=Path('acceleration/results/20260917_independent_review/four_matchings/summary.json')
AUDIT_HASH='fb21f69b6e5785e0897bccb6965bfea75c171aad2e685b7d60966bc5c8019e73'
PROTOCOL=Path('docs/NEXT_20260917_SIX_COORDINATE_DOMAIN_PILOT.md')
DOMAIN='PARTIAL_K_SIX_SAME_SIGN_COORDINATES_ROOT012_V1'
ORDER=[0,4,24,8,60]+[u for u in range(84) if u not in(0,4,24,8,60)]
...
def main():
    ...
    assert audit['status']=='INDEPENDENT_FOUR_COORDINATE_DOMAINS_PASS' and audit['domain_choices']==290460
    ...
    assert len(fixed)==132 and len(removed)==36 and len(allowed_Y)==360 and len(unknown)==2040
    assert Counter(14-rows[u+15].bit_count() for u in range(84))=={10:12,9:48,8:24}
    assert fixed<=old_fixed and valid(rows)
    scope='Retain132baselineK edges including all7cross and8other same-sign matchings; all same-fibre and unlisted other-coordinate absences fixed. Unknown360same-sign edges atrootgroups0,1,2 plus1680disjoint-support edges.'
    manifest=dict(timestamp=stamp(),source_commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
        ...
        domain=DOMAIN,scope=scope,enumeration_order=ORDER,selection='Three double-freed support pairs0/4/24, one single-freed8, one unaffected60, then remaining84labels ascending; fixed before results.',
        limits=dict(enumeration_seconds=180,nodes=20000000,per_center_domains=100000,total_domains=1000000),
        ...
        unknown_edges_outer=unknown,missing_neighbors='10at12centers,9at48centers,8at24centers',
        old_population=290460,old_embedding='Add removed old fixedneighbors to eachmask; fullcenter neighborhood identical; otheredges onlydeleted, preserving localcaps/rootquotas.',
        ...
        controls='Five preselected restricteduniverses brutechecked, corruptedmissingedge rejected; cap0 retains trigger and no completeflag.',
    ...
    for u in tqdm(range(84),desc='Embed all prior four-coordinate stars',unit='center'):
    ...
    assert sum(x['old_choices'] for x in embedding_counts)==290460
    save(args.out/'embedding_summary.json',dict(status='CANDIDATE_ALL_OLD_NEIGHBORHOODS_EMBED',old_choices=290460,records=embedding_counts,
    ...
    started=time.monotonic();budget=dict(deadline=started+180,nodes=0,node_cap=20000000,domain_cap=100000,total_cap=1000000,complete_choices=0)
    records=[];incomplete=None
    for u in tqdm(ORDER,desc='Six freed matching coordinates',unit='center'):
        ...
        record=dict(outer_vertex=u,domain=DOMAIN,status='CANDIDATE_COMPLETE_SIX_COORDINATE_DOMAIN',domain_size=len(masks),
    ...
    summary=dict(timestamp=stamp(),status='CANDIDATE_SIX_COORDINATE_DOMAIN_PILOT',domain=DOMAIN,scope=scope,
        ...
        all_old_neighborhood_embeddings=290460,old_choices_located_in_complete_domains=sum(r['old_embedded_choices'] for r in records),
