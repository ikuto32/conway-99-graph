...
__constant__ short x_index[V*V],quota_index[V*14],pair_index[V*V],quota_columns[Q*8];
__constant__ unsigned char x_u[N],x_v[N],pair_u[P],pair_v[P],vertex_labels[V*2];
...
__device__ __forceinline__ double read_x(const double* x,int u,int v){
    const int i=x_index[u*V+v];return i<0?0.0:x[i];
}
__device__ __forceinline__ double forward_row(int row,const double* x,const unsigned char* neighbors){
    double value=0;
    if(row<Q){
        #pragma unroll
        for(int k=0;k<8;++k)value+=x[quota_columns[row*8+k]];
    }else{
        const int p=row-Q,u=pair_u[p],v=pair_v[p];
        value=read_x(x,u,v);
        #pragma unroll
        for(int k=0;k<4;++k){
...
__device__ __forceinline__ double transpose_column(int column,const double* y,const unsigned char* neighbors){
    const int u=x_u[column],v=x_v[column];
    double value=y[quota_index[u*14+vertex_labels[v*2]]];
    value+=y[quota_index[u*14+vertex_labels[v*2+1]]];
    value+=y[quota_index[v*14+vertex_labels[u*2]]];
    value+=y[quota_index[v*14+vertex_labels[u*2+1]]];
    value+=y[Q+pair_index[u*V+v]];
    #pragma unroll
    for(int k=0;k<4;++k){
        value+=y[Q+pair_index[v*V+neighbors[u*4+k]]];
        value+=y[Q+pair_index[u*V+neighbors[v*4+k]]];
    }
    return value;
}
...
int main(int argc,char** argv){try{
    ...
    CUDA(cudaMemcpyToSymbol(x_index,xi.data(),xi.size()*sizeof(short)));
    CUDA(cudaMemcpyToSymbol(quota_index,qi.data(),qi.size()*sizeof(short)));
    CUDA(cudaMemcpyToSymbol(pair_index,pi.data(),pi.size()*sizeof(short)));
    CUDA(cudaMemcpyToSymbol(quota_columns,qcols.data(),qcols.size()*sizeof(short)));
    CUDA(cudaMemcpyToSymbol(x_u,xu.data(),N));CUDA(cudaMemcpyToSymbol(x_v,xv.data(),N));
    CUDA(cudaMemcpyToSymbol(pair_u,pu.data(),P));CUDA(cudaMemcpyToSymbol(pair_v,pv.data(),P));
    CUDA(cudaMemcpyToSymbol(vertex_labels,labels.data(),labels.size()));
