"""240-second replay of the unchanged independently audited full-moment model."""
import argparse
...
def main():
    ...
    files=[SOURCE+"/model.json",SOURCE+"/integer_augmented_csr.npz",SOURCE+"/numeric_lp.json",SOURCE+"/manifest.json",SOURCE+"/highs.log",AUDIT,"uv.lock",
    ...
    save(args.out/"manifest.json",dict(timestamp=datetime.now(timezone.utc).isoformat(),
        ...
        selection="60-second IPM log had positive internal dual objective but incomplete residual convergence and no valid exported solution. Arbitrary raw-weight exact bound was negative. Longer same-model run may yield useful weights; no guarantee or speed claim.",
        limits=dict(solver_seconds=240,solver="ipm",threads=1,crossover="off",expected_wall="About four minutes plus setup; solver iteration granularity can exceed cap"),
    ...
    for key,value in (("output_flag",True),("threads",1),("solver","ipm"),("run_crossover","off"),("time_limit",240.0),("log_file",str(args.out/"highs.log"))):
    ...
    result=dict(timestamp=datetime.now(timezone.utc).isoformat(),status="CANDIDATE_SAME_MODEL_240S_REPLAY",
