...
def main():
    ...
    require(summary['status'] == 'BOUNDED_CP_MATCHING_SEARCH_FINISHED' and
            audit['status'] == 'INDEPENDENT_CP_CROSS_SEARCH_AUDIT_PASS', 'Search not independently audited')
    ...
    sources = [Path(__file__), ROOT/'acceleration/check_shortlist_native_pair.py',
               ROOT/'acceleration/audit_goal_theory_pairs.py', ROOT/'acceleration/bind_cp_cross_improvement.py']
    ...
    for index in ordered:
        ...
        if gate['passed']:
            ...
            if attempt['independently_nonempty']:
                ...
                run('bind_cp_cross_improvement.py', ['--summary', key(summary_path), '--search-audit', key(audit_path),
