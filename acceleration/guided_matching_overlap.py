"""Whole same-sign matching search with CUDA ranking and native pair AC.

Rust completely enumerates replacements in each selected same-sign coordinate.
The default extended shortlist uses five/six changed edges or multiple cycles;
all moves may instead be eligible. Only final graphs need satisfy the caps.
GPU scores are fixed-X heuristic upper bounds, never exclusion certificates.
Numerical zero is a stopping point for exact validation, not a Conway graph.
Derived from the frozen guided_atomic_overlap.py; historical code is preserved.
"""
import argparse
import json
...
def main():
    ...
    parser.add_argument('--coordinates', default='all')
    parser.add_argument('--move-scope', choices=('all','extended'), default='extended')
    ...
    require(args.coordinates == 'all' or args.coordinates in {f'{g}:{sign}' for g in range(7) for sign in range(2)}, 'Invalid matching coordinates')
    ...
    sources = [args.initial, Path(__file__), native, gpu, stars, pairs,
               ROOT/'acceleration/guided_global_overlap.py', ROOT/'acceleration/guided_atomic_overlap.py',
               ...
               ROOT/'acceleration/phase1_probe_ipm.py', ROOT/'acceleration/overlap_matching_neighbors.rs',
    ...
    manifest = dict(inputs_sha256=bindings, seed=args.seed, iterations=args.iterations,
                    coordinates=args.coordinates, move_scope=args.move_scope, lp_best=args.lp_best, lp_diverse=args.lp_diverse,
                    lp_random=args.lp_random, cached_best=args.cached_best,
                    seconds_per_lp=args.seconds_per_lp,
                    scope='Bounded global-defect search over completely enumerated whole same-sign matching coordinates, with a declared LP eligibility filter and native pair AC at accepted endpoints. No intermediate graph is asserted. LP shortlist is nonexhaustive; cached native failures only guide selection. Caps are unavailable checks, never exclusions. No full graph or exhaustive coverage.')
    ...
    for iteration in range(args.iterations):
        ...
        if reused:
            ...
            proposed = invoke(native, neighbor_input, neighbor_output, args.coordinates)
            ...
            require(proposed['status'] == 'COMPLETE_WHOLE_SAME_SIGN_MATCHING_SUBFAMILY_ENUMERATION'
                    and len(proposed['moves']) == len(options)
                    and all(len(move['removed']) == len(move['added']) == move['changed_edges']
                            and 2 <= move['changed_edges'] <= 6
                            and sum(len(c)//2 for c in move['alternating_cycles']) == move['changed_edges']
                            for move in proposed['moves']),
                    'Invalid whole-matching proposal batch')
        ...
        if not options:
            stop = 'NO_LEGAL_WHOLE_MATCHING_REPLACEMENTS'
            break
        eligible = [i for i, key in enumerate(option_keys) if key not in visited and
                    (args.move_scope == 'all' or proposed['moves'][i]['changed_edges'] >= 5
                     or len(proposed['moves'][i]['alternating_cycles']) > 1)]
        ...
        def bucket(i):
            ...
            return (move['root_group'], move['matching_class'], move['changed_edges'],
                    tuple(sorted(len(c)//2 for c in move['alternating_cycles'])))
        ...
        if not selected:
            stop = 'NO_UNPROBED_OR_ELIGIBLE_CACHED_MATCHING_CANDIDATES'
            break
        ...
        record = dict(iteration=iteration, proposal_batch_reused=reused,
                      eligible_after_move_scope_and_visited=len(eligible),
        ...
        summary = dict(status='SEARCH_IN_PROGRESS', best_numeric_objective=best['objective'],
                       ...
                       accepted_edge_replacements=sum(row['move']['changed_edges'] for row in records if row['accepted']),
    ...
    write_json(args.out/'best_candidate.json', {'overlap_edges_outer_zero_based': best['candidate'],
               ...
               'source_manifest': path_key(args.out/'manifest.json')})
    write_json(args.out/'best_phase1.json', best['data'])
    summary = dict(status='BOUNDED_WHOLE_MATCHING_COUPLED_SEARCH_FINISHED', stop_reason=stop,
