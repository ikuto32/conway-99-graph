...
def verify_request(request, family, family_audit, read, bind):
    mapping_path = 'acceleration/results/20260917_independent_review/whole_fresh_balanced_mapping.json'
    mapping = read(mapping_path)
    require(bind(mapping_path) == '62373d4c8607baf3103b5bb98ad59427e6e221833b3dba038259d38459ff0eb8', 'Frozen independent mapping report changed')
    require(mapping['status'] == 'INDEPENDENT_WHOLE_FRESH_V2_MAPPING_PASS' and
            mapping['policy_version'] == 'BALANCED_COORDINATES_V2B' and
            mapping['selected_indices'] == request['indices'], 'Whole mapping association')
    require(request['status'] == 'EXPLICIT_WHOLE_FRESH_STAR_COHORT_REQUEST_V2' and
            request['objective_id'] == 'ORIGINAL_STAR_SIMPLEX_PDHG_V1' and
            family_audit['status'] == 'INDEPENDENT_COMPLETE_WHOLE_SAME_SIGN_MATCHING_FAMILY_PASS' and
            family['legal_count'] == len(family['overlap_candidates']) == 81000, 'Whole family scope')
    for f in ('acceleration/rank_fresh_whole_v2_balanced.py', request['family_path'],request['family_audit_path']):
        require(mapping['inputs_sha256'][key(f)] == bind(f), 'Mapping source binding')
    return dict(selected_indices=request['indices'], independent_mapping_path=mapping_path,
                independent_mapping_sha256=bind(mapping_path), policy_version='BALANCED_COORDINATES_V2B')
...
def audit_ranking(summary_path, cpu_controls=3):
    ...
    bind(__file__)
    bind(ROOT/'acceleration/audit_20260917_ranking_model.py')
    bind(ROOT/'acceleration/rank_fresh_whole_v2_balanced.py')
    summary = read(summary_path)
    require(summary['status'] == 'NUMERICAL_WHOLE_FRESH_STAR_PDHG_RANKING_FINISHED'
            and summary['producer_version'] == 'whole_fresh_v2_balanced' and summary['objective_id'] == 'ORIGINAL_STAR_SIMPLEX_PDHG_V1', 'Unfinished or unsupported ranking')
    ...
    require(manifest['status'] == 'WHOLE_FRESH_STAR_PDHG_V2_INPUTS_BOUND'
    ...
    for position, (index, row, native_row) in enumerate(zip(indices, summary['records'], native['results'])):
        require(row['proposal_index'] == index and row['native_result_index'] == position
                and row['original_native_index'] == index
                and row['root_group'] == request['records'][position]['root_group']
                and row['matching_class'] == request['records'][position]['matching_class']
                and row['changed_edges'] == request['records'][position]['changed_edges']
                and row['alternating_cycle_sizes'] == request['records'][position]['alternating_cycle_sizes']
    ...
    return dict(status='INDEPENDENT_WHOLE_FRESH_STAR_PDHG_RANKING_AUDIT_PASS', inputs_sha256=bindings,
        ...
        producer_or_native_imported=False, producer_model_builder_used=False,
        model_reconstruction_method='Dense integer B, direct derivative BE+EB+E, original-mask incidence',
        shared_trusted_components=['NumPy/SciPy exact small-integer sparse storage','existing independent binary parser/bounds','CPU simplex projection shared with historical producer model library; builder not used','hash-bound mapping audit'],
        independent_domain_enumeration_performed=False,
