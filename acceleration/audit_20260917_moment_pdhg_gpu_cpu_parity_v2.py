...
ROOT=h.ROOT;D=ROOT/'acceleration/results/20260917_moment_pdhg_gpu';OUT=ROOT/'acceleration/results/20260917_independent_review/moment_pdhg_gpu_cpu_parity_v2'
VTOL=1e-8;STOL=1e-7
...
def main():
    ...
    gate=read(ROOT/'acceleration/results/20260917_independent_review/two_matching_moments.json');h.require(h.digest(ROOT/'acceleration/results/20260917_independent_review/two_matching_moments.json')=='5af1f353a70a16fc5f915195b911d782e360ffc0690463936b7a9dea983197ed','audited model gate')
    ...
    for name,record in manifest['records'].items():
        ...
        for iteration in tqdm(range(1,101),desc='Independent CPU '+name,unit='iteration'):
            ...
            for k,v in vectors.items():
                observed=np.asarray(gpu[k],dtype=np.float64);h.require(observed.shape==v.shape and np.isfinite(observed).all(),'finite vectors');err=float(np.max(np.abs(v-observed)));errors[k]=err;h.require(err<=VTOL,'vector parity '+name+' '+k)
