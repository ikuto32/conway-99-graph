...
CHECKER='acceleration/audit_20260917_two_matching_bound.py'
REPORT='acceleration/results/20260917_independent_review/two_matching_solve_bound.json'
REPORT_HASH='adf254892686dab05f0317b9ecec963b1b98eee8f9f8ccee129b05ebf6145065'
def require(ok,why):
...
def main():
    ...
    try:
        ...
        require(module.ROOT.resolve()==root,'Imported scientific root mismatch');sys.argv=[str(root/CHECKER),'--mode','solve'];module.main()
        result=json.loads(out.read_bytes());require(result['status']=='INDEPENDENT_TWO_COORDINATE_EXACT_SUPPORT_BOUND_PASS','Unexpected checker result')
        require(result['exact_bound']==published['exact_bound']and result['checked_choices']==89308 and result['conditional_family_exclusion']is True,'Scientific replay result mismatch');status='PASS'
