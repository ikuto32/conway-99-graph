...
from audit_matching_accepted import check_alternating_cycle
from audit_atomic_accepted import check_atomic_geometry
...
def main():
    ...
    require(search_audit['status'] == 'INDEPENDENT_CP_CROSS_SEARCH_AUDIT_PASS', 'Search audit incomplete')
    ...
    require(record['move']['matching_class'] == 'cross' and record['move']['cycle_size'] in (3, 4),
            'Expected one cross-class three/four-edge cycle')
    check_alternating_cycle(record['move'])
    check_atomic_geometry(set(map(tuple, previous['overlap_edges_outer_zero_based'])),
    ...
    sources = [args.summary, args.search_audit, args.previous_candidate, args.previous_phase1, candidate_path, phase_path,
               ...
               ('audit_certificate.py', 'audit_phase1.py', 'audit_phase1_kkt.py', 'audit_matching_accepted.py', 'audit_atomic_accepted.py')]
