"""One bounded continuous completion check for the audited overlap walk."""
import argparse
...
def main():
    ...
    parser.add_argument('--input',type=Path,default=Path('scratch_follow_overlap_walk.json'))
    path=(HERE/parser.parse_args().input).resolve()
    assert HERE in path.parents
    assert not path.with_name(path.stem+'_lp.json').exists(), 'preserve prior bounded result'
    audit=json.loads(path.with_name(path.stem+'_audit.json').read_bytes())
    assert audit['status']=='INDEPENDENT_OVERLAP_WALK_AND_640_COMPILED_CUT_AUDIT_PASS'
    assert audit['candidate_sha256']==sha256(path.read_bytes()).hexdigest()
    ...
    status=solver.Solve()
    result={'candidate_path':path.name,'candidate_sha256':sha256(path.read_bytes()).hexdigest(),
