"""Exhaustive direct-matching check of every E74 group signature domain."""
...
import scratch_general_e78_port_audit as direct


INPUT_PATH = Path("scratch_general_e74_compression_audit.json")
OUTPUT_PATH = Path("scratch_general_e74_port_group_check.json")
...
def main():
    ...
    for layout in sorted(layouts):
        domains = [range(len(port.FIBRE_STATES[deficit])) for deficit, _axis in layout]
        seen = set()
        assignments = 0
        hall_positive = 0
        for state_indices in itertools.product(*domains):
            assignments += 1
            signatures = tuple(
                port.SIGNATURES[deficit][state_index][axis]
                for (deficit, axis), state_index in zip(layout, state_indices)
            )
            if signatures in seen:
                continue
            seen.add(signatures)
            hall = port.group_matchable_signatures(signatures)
            matching = direct.direct_matching_exists(synthetic_ports(signatures))
            assert hall == matching
            hall_positive += hall
    ...
    result = {
        "status": "VERIFIED",
        "model": "all E0=74 root-group signature domains: Hall formula versus direct perfect-matching DFS",
    ...
    print(json.dumps({key: result[key] for key in (
        "status", "distinct_incident_deficit_axis_layouts",
        "labelled_group_state_assignments_covered", "unique_signature_rows_checked"
    )}))
