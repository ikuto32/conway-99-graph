"""One30-second exact-capacity-certificate attempt for the overlap walk."""
import argparse
...
def main():
    ...
    assert not path.with_name(path.stem+'_farkas.json').exists(), 'preserve prior bounded result'
    ...
    status=solver.Solve()
    result={'status':'NO_EXACT_CERTIFICATE','solver_status_code':status,'candidate_path':path.name,
